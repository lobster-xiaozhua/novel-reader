...
class RobotsTxtChecker:
    def __init__(self):
        ...
        self._user_agent = "NovelReaderBot/1.0"

    async def can_fetch(self, target_url: str, session: aiohttp.ClientSession) -> bool:
        parsed = urlparse(target_url)
        robots_url = f"{parsed.scheme}://{parsed.netloc}/robots.txt"

        if robots_url not in self._cache:
            self._cache[robots_url] = await self._fetch_robots(robots_url, session)

        rules = self._cache[robots_url]
        if not rules.get("fetched", False):
            return True
        ...
        for pattern in disallowed:
            ...
            if path.startswith(pattern):
                crawl_delay = rules.get("crawl_delay", 0)
                if crawl_delay > 0:
                    logger.info(f"robots.txt 要求延迟 {crawl_delay} 秒")
                return False
        return True
    ...
    async def get_crawl_delay(self, target_url: str) -> float:
        parsed = urlparse(target_url)
        robots_url = f"{parsed.scheme}://{parsed.netloc}/robots.txt"
        rules = self._cache.get(robots_url, {})
        return rules.get("crawl_delay", 0)

    async def _fetch_robots(self, robots_url: str, session: aiohttp.ClientSession) -> Dict:
        try:
            async with session.get(robots_url, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                if resp.status != 200:
                    return {"fetched": False}
                text = await resp.text()
                return self._parse_robots(text)
        except Exception as e:
            logger.debug(f"获取 robots.txt 失败: {robots_url}, {e}")
            return {"fetched": False}
    ...
    def _parse_robots(self, text: str) -> Dict:
        ...
        for line in text.splitlines():
            ...
            if directive == "user-agent":
                current_agent = value == "*" or value == self._user_agent
            elif current_agent:
        ...
        return {"fetched": True, "disallow": disallow, "crawl_delay": crawl_delay}
...
class CrawlerService:
    ...
    async def run_task(self, task_id: int):
        async with AsyncSessionLocal() as db:
            try:
                ...
                task.status = "running"
                task.logs = json.dumps([{"time": time.time(), "msg": "任务开始执行"}])
                await db.commit()
                ...
                logger.error(f"爬虫任务 {task_id} 执行失败: {e}", exc_info=True)
                try:
                    from sqlalchemy import select
                    result = await db.execute(select(CrawlerTask).where(CrawlerTask.id == task_id))
                    task = result.scalar_one_or_none()
                    if task:
                        ...
                        await self._append_log(db, task, f"任务失败: {e}")
                        await db.commit()
                except Exception:
                    pass
            finally:
    ...
    async def _execute_crawl(self, db, task: CrawlerTask):
        ...
        async with aiohttp.ClientSession(timeout=timeout, connector=connector) as session:
            if not await self.robots_checker.can_fetch(task.url, session):
                task.status = "failed"
                task.error_message = "目标网站 robots.txt 禁止爬取"
                await self._append_log(db, task, "robots.txt 禁止爬取")
                await db.commit()
                return

            crawl_delay = await self.robots_checker.get_crawl_delay(task.url)
            base_delay = max(settings.CRAWLER_REQUEST_DELAY, crawl_delay)

            concurrency_ctrl = DynamicConcurrencyController(
                max_concurrent=settings.CRAWLER_MAX_CONCURRENT,
                base_delay=base_delay,
            )

            await self._append_log(db, task, f"开始解析目录页: {task.url}")
            ...
            if not chapter_list:
                ...
                await self._append_log(db, task, "章节列表解析失败")
                await db.commit()
                return

            task.total_chapters = len(chapter_list)
            await self._append_log(db, task, f"解析到 {len(chapter_list)} 个章节")
            await db.commit()
            ...
            if start_index > 0:
                await self._append_log(db, task, f"断点续传，从第 {start_index + 1} 章开始")

            books_dir = Path(settings.BOOKS_DIR) / self._safe_filename(book_info.get("title") or f"book_{book.id}")
            books_dir.mkdir(parents=True, exist_ok=True)

            for i in range(start_index, len(chapter_list)):
                if not self._active_tasks.get(task.id, False):
                    task.status = "cancelled"
                    await self._append_log(db, task, "任务已取消")
                    await db.commit()
                    return

                chapter_info = chapter_list[i]
                delay = concurrency_ctrl.get_delay()
                ...
                try:
                    ...
                    if chapter_html:
                        ...
                        if content:
                            ...
                            await self._save_chapter(db, book.id, i + 1, title, str(chapter_path), word_count)

                            task.downloaded_chapters = i + 1
                            concurrency_ctrl.record_response(elapsed, True)
                        else:
                            concurrency_ctrl.record_response(elapsed, False)
                            await self._append_log(db, task, f"第 {i + 1} 章内容为空: {chapter_info['title']}")
                    else:
                        concurrency_ctrl.record_response(0, False)
                        await self._append_log(db, task, f"第 {i + 1} 章获取失败: {chapter_info['title']}")
                    ...
                    concurrency_ctrl.record_response(0, False)
                    await self._append_log(db, task, f"第 {i + 1} 章处理异常: {e}")
                finally:
                    concurrency_ctrl.release()

            book.total_chapters = task.downloaded_chapters
            task.status = "completed"
            await self._append_log(db, task, f"任务完成，共下载 {task.downloaded_chapters} 章")
            await db.commit()
    ...
    async def _fetch_page(self, session: aiohttp.ClientSession, url: str) -> Optional[str]:
        ...
        try:
            async with session.get(url, headers=headers) as resp:
                ...
                logger.warning(f"HTTP {resp.status} 获取页面失败: {url}")
                return None
        except (aiohttp.ClientError, asyncio.TimeoutError) as e:
            logger.warning(f"请求异常: {url}, {e}")
            raise

    async def _ensure_book(self, db, book_info: Dict, task: CrawlerTask) -> Book:
        from sqlalchemy import select
        ...
        if not book:
            ...
            book = Book(
                ...
                folder_path=str(Path(settings.BOOKS_DIR) / safe_name),
                total_chapters=0,
            )
            db.add(book)
            await db.flush()
    ...
    async def _save_chapter(self, db, book_id: int, chapter_number: int, title: str, file_path: str, word_count: int):
        from sqlalchemy import select
    ...
    async def _append_log(self, db, task: CrawlerTask, message: str):
        try:
