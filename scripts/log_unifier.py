...
class LogEntry:
    ...
    def format(self, show_source=True, show_time=True):
        ...
        if show_source:
            ...
            source_icon = {'backend': '[B]', 'frontend': '[F]', 'system': '[S]'}.get(self.source, '?')
        ...
        level_icon = {'INFO': 'i', 'DEBUG': '-', 'WARN': '!', 'ERROR': 'x'}.get(self.level, '?')
...
class SmartLogViewer:
    ...
    def print_header(self):
        ...
        print(f"{COLORS['bold']}  {COLORS['cyan']}Novel Reader - 智能日志监控{COLORS['reset']}")
        print(f"{COLORS['bold']}  {COLORS['dim']}按 Ctrl+C 退出 | 按 n 切换噪声过滤 | 按 s 切换统计{COLORS['reset']}")
    ...
    def print_stats_bar(self):
        ...
        if recent_errors:
            ...
            for err in recent_errors:
                print(f"    {COLORS['error']}x {err.message[:60]}{COLORS['reset']}")
