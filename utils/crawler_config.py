...
class SiteConfig:
    name: str
    domain: str
    chapter_list_selectors: List[str] = field(
        default_factory=lambda: ["div[class*='list']", "div[class*='chapter']", "dl", "ul", "ol"]
    )
    content_selectors: List[str] = field(
        default_factory=lambda: ["#content", "#bookContent", ".content", ".chapter-content", ".read-content"]
    )
    ...
    skip_keywords: List[str] = field(
        default_factory=lambda: ["首页", "末页", "上一页", "下一页", "返回", "目录", "顶部", "底部"]
    )
